...
export const dedupe = (items: Model[]) => {
  ...
  for (const m of items) {
    ...
    const nameKeyRaw = (m && (m as any).name) ? String((m as any).name) : ((m as any).id ? String((m as any).id) : "");
    const nameKey = normalizeNameForMatch(nameKeyRaw) || nameKeyRaw.toLowerCase();
    const providerRaw = (m as any).provider;
    const providerKey = (providerRaw ? String(providerRaw) : '').toLowerCase().replace(/[^a-z0-9]/g, '');
...
export function mapDomain(v?: any, sheetName?: string): Domain {
