...
export const TerminalConsole: React.FC<TerminalConsoleProps> = ({ logs, onClose, onClear }) => {
  ...
  const levelBadge = (level?: ParsedEntry['level']) => {
    ...
    switch (level) {
      case 'INFO': return <span className={`${base} bg-blue-500/10 text-blue-500`}>INFO</span>;
      case 'WARN': return <span className={`${base} bg-yellow-500/10 text-yellow-500`}>WARN</span>;
      case 'ERROR': return <span className={`${base} bg-red-500/10 text-red-500`}>ERROR</span>;
      case 'WEBHOOK': return <span className={`${base} bg-emerald-500/10 text-emerald-500`}>WEBHOOK</span>;
      default: return <span className={`${base} bg-bg-input text-text-subtle`}>LOG</span>;
    }
  };

  return (
    <div className="fixed bottom-4 left-4 z-40 w-[720px] max-h-[75vh] bg-bg-card text-text border border-border rounded-xl shadow-2xl flex flex-col">
      <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-bg-card rounded-t-xl">
        ...
        <div className="flex items-center gap-2">
          <button className="text-xs px-2 py-1 bg-bg-input rounded hover:bg-bg-elevated" onClick={onClear}>Clear</button>
          <button className="text-xs px-2 py-1 bg-bg-input rounded hover:bg-accent hover:text-white" onClick={onClose}>Close</button>
        </div>
      </div>
            ...
            {groups.map(({ id, items }) => {
              ...
              if (isNetwork) {
                ...
                return (
                  <div key={id} className="border border-border rounded-lg overflow-hidden">
                    <div className="px-3 py-2 bg-bg-card flex items-center gap-2">
                      <span className="text-[10px] text-text-subtle">{id}</span>
                      {req?.method && <span className="px-1.5 py-0.5 rounded bg-bg-input text-text-secondary text-[10px]">{req.method}</span>}
                      ...
                      {err && !res && <span className="ml-auto text-xs text-red-500">Error</span>}
                    </div>
                    <div className="grid grid-cols-2 gap-0">
                      <div className="p-2 border-r border-border">
                        <div className="text-[10px] text-text-subtle mb-1">Request</div>
                        {prettyHeaders && (
                          <details>
                            <summary className="cursor-pointer text-[10px] text-text-secondary">Headers</summary>
                            <pre className="whitespace-pre-wrap text-[10px] text-text">{prettyHeaders}</pre>
                          </details>
                        )}
                        {req?.body && (
                          <details>
                            <summary className="cursor-pointer text-[10px] text-text-secondary">Body</summary>
                            <pre className="whitespace-pre-wrap text-[10px] text-text">{prettyReqBody || req.body}</pre>
                          </details>
                        )}
                        {!req?.body && !prettyHeaders && <div className="text-[10px] text-text-secondary">—</div>}
                      </div>
                      <div className="p-2">
                        <div className="text-[10px] text-text-subtle mb-1">Response</div>
                        {res?.body && (
                          <details>
                            <summary className="cursor-pointer text-[10px] text-text-secondary">Body</summary>
                            <pre className="whitespace-pre-wrap text-[10px] text-text">{prettyResBody || (res.body?.slice(0, 4000) || '')}</pre>
                          </details>
                        )}
                        {err?.body && (
                          <div className="text-red-500 text-[10px] whitespace-pre-wrap">{err.body}</div>
                        )}
                        {!res?.body && !err?.body && <div className="text-[10px] text-text-secondary">—</div>}
                      </div>
                    </div>
                  </div>
                );
              ...
              return (
                <div key={id} className="border border-border rounded-lg overflow-hidden">
                  <div className="px-3 py-2 bg-bg-card flex items-center gap-2">
                    <span className="text-[10px] text-text-subtle">{id}</span>
                    {entry.kind === 'APP' ? levelBadge(entry.level) : <span className="px-1.5 py-0.5 rounded bg-bg-input text-text-subtle text-[10px]">LOG</span>}
                    <span className="text-xs truncate flex-1">{(entry.body && entry.body.split('\n')[0]) || entry.raw}</span>
                  </div>
                  {entry.body && entry.body.includes('\n') && (
                    <div className="p-2">
                      <details>
                        <summary className="cursor-pointer text-[10px] text-text-secondary">Details</summary>
                        <pre className="whitespace-pre-wrap text-[10px] text-text">{entry.body}</pre>
                      </details>
                    </div>
                  )}
                </div>
